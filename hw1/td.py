...
def epsilon_greedy_policy(nS, nA, Q_function, eps=0.5):
    ...
    policy = np.ones((nS, nA)) * eps / nA
    policy[np.arange(nS), Q_function.argmax(axis=1)] = 1 - eps + eps / nA
...
def Q_learning_step(Q_function, state, action, reward, next_state, next_action, terminal, alpha, gamma):
    ...
    next_Q_function[state, action] = Q_function[state, action] + alpha * (reward + gamma * np.max(Q_function[next_state, :]) * (1 - terminal) - Q_function[state, action])
...
def Sarsa_step(Q_function, state, action, reward, next_state, next_action, terminal, alpha, gamma):
    ...
    next_Q_function = Q_function.copy()
    next_Q_function[state, action] = Q_function[state, action] + alpha * (reward + gamma * Q_function[next_state, next_action] * (1 - terminal) - Q_function[state, action])
