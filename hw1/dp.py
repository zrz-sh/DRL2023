    ...
    while True:
      for s in range(nS):
        ...
        for i in range(len(P[s][policy[s]])):
          p, next_s, r, done = P[s][policy[s]][i]
          temp_value += p * (r + gamma * prev_value_function[next_s] * (1 - done))
        value_function[s] = temp_value
    ...
    for s in range(nS):
      ...
      for a in range(nA):
        ...
        for i in range(len(P[s][a])):
          p, next_s, r, done = P[s][a][i]
          temp_value += p * (r + gamma * value_from_policy[next_s] * (1 - done))
        temp_value_list.append(temp_value)
      new_policy[s] = np.argmax(temp_value_list)
      ...
      for s in range(nS):
        ...
        for a in range(nA):
          ...
          for i in range(len(P[s][a])):
            ...
            temp_value += p * (r + gamma * prev_value_function[next_s] * (1 - done))
          temp_value_list.append(temp_value)
        value_function[s] = max(temp_value_list)
      if np.max(np.abs(value_function - prev_value_function)) < eps:
        break
      prev_value_function = np.copy(value_function)
    ...
    for s in range(nS):
      ...
      for a in range(nA):
        ...
        for i in range(len(P[s][a])):
          ...
          temp_value += p * (r + gamma * prev_value_function[next_s] * (1 - done))
        temp_value_list.append(temp_value)
      policy[s] = np.argmax(temp_value_list)      
