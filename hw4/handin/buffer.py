...
class ReplayBuffer:
    ...
    def sample(self, batch_size):
        ...
        ############################
        # YOUR IMPLEMENTATION HERE #
        batch = (self.state[sample_idxs].to(self.device), 
...
class PPOReplayBuffer(ReplayBuffer):
    ...
    def get_next_values(self, agent, t) -> torch.Tensor:
        ...
        ############################
        # YOUR IMPLEMENTATION HERE #
        if t==self.capacity-1 or self.done[t].any():
    ...
    def compute_advantages_and_returns(self, agent) -> None:
        ...
        for t in reversed(range(self.capacity)):
            next_values = torch.tensor(self.get_next_values(agent, t),device=self.device)
            ...
            if t == self.capacity-1:
                ...
                self.advantage[t] = self.reward[t] + self.gamma * next_values * (1 - self.done[t]) - self.value[t] + self.gamma * self.gae_lambda * self.advantage[t+1] * (1 - self.done[t])
            ############################
        # calculate the returns
        ############################
        # YOUR IMPLEMENTATION HERE #
        self.returns = self.advantage + self.value # TD target
        ############################
