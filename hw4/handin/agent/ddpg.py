...
class DDPGAgent:
    ...
    def get_Qs(self, state, action, reward, next_state, done) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        with torch.no_grad():
            Q_target = reward + (1 - done) * self.gamma * self.critic_target(next_state, self.actor_target(next_state))
        ############################
        return Q, Q_target
    ...
    def get_action(self, state, sample=False):
        ...
        ############################
        # YOUR IMPLEMENTATION HERE #
        action = self.actor_net(torch.tensor(state).to(self.device))
        if sample:
            action += self.eps_schedule(self.train_step) * torch.randn_like(action)
            action = torch.clamp(action, min=self.actor_net.action_space.low, max=self.actor_net.action_space.high)
