...
class SACAgent(TD3Agent):
    ...
    def get_Qs(self, state, action, reward, next_state, done) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        with torch.no_grad():
            ...
            next_Q = torch.min(self.critic_target(next_state, next_action), self.critic_target_2(next_state, next_action)) - torch.exp(self.log_alpha) * next_log_prob
    ...
    def get_actor_loss(self, state) -> tuple[torch.Tensor, torch.Tensor]:
        """
        ...
        actor_loss = (torch.exp(self.log_alpha) * log_prob - self.critic_net(state, action)).mean()
        ############################
